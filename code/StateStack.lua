...
function StateStack:PushFixedBox(renderer, x, y, width, height, text, params)
...
function StateStack:PushFittedBox(renderer, x, y, text, wrap, params)
    ...
    return self.PushFixedBox(renderer, x, y, width, height, text, params)
end


--
-- StateStack Statemachine code
--
...
function StateStack:Update(dt)
	...
	-- top state update first => work backwards
	...
	-- don't update other states below top
	if not top then
		return
	end

	-- input handled by top state, not stack
	top:HandleInput()
end